...
void robotPosition(const move_base_msgs::MoveBaseActionFeedback::ConstPtr& msg) {
	...
	if(abs(e_x) <= goal_th && abs(e_y) <= goal_th){
		...
		pubGoalStatus.publish(status_msg);
		return;
	}
	...
	if (time > timeout) {
		...
		canc_goal.id = id;
		pubCanc.publish(canc_goal);
		status_msg.data = "Goal Not Reachable";
		pubGoalStatus.publish(status_msg);
		return;

	}
	
	status_msg.data = "Goal In Progress";
	pubGoalStatus.publish(status_msg);
...
int main(int argc, char** argv){
	...
	pubGoalStatus = n.advertise<std_msgs::String>("goal_status", 10);
